...
if(!fs.existsSync(YTDLP_PATH)){
    console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⏳ yt-dlp.exe not found — downloading automatically...');
    try{
        ...
        if(fs.existsSync(YTDLP_PATH)){
            console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ✅ yt-dlp.exe downloaded successfully!');
        } else {
            console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⚠️  yt-dlp.exe download failed — !dl and !insta will not work');
        }
    }catch(e){
        console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⚠️  yt-dlp.exe auto-download failed:', e.message);
        console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT]    Download manually: https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe');
    }
} else {
    console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ✅ yt-dlp.exe found — !dl and !insta ready');
}

const FFMPEG_PATH = './ffmpeg.exe';
if(!fs.existsSync(FFMPEG_PATH)){
    console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⏳ ffmpeg.exe not found — downloading automatically for video rendering...');
    try{
        const { execSync } = await import('child_process');
        ...
        if(fs.existsSync(FFMPEG_PATH)){
            console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ✅ ffmpeg.exe installed successfully!');
        } else {
            console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⚠️  ffmpeg.exe installation failed');
        }
    }catch(e){
        console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⚠️  ffmpeg.exe auto-download failed:', e.message);
    }
} else {
    console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ✅ ffmpeg.exe found — high-quality rendering ready');
}
...
if(!HAS_FFMPEG) console.warn('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ⚠️  ffmpeg not found — voice notes will play on web only. Install ffmpeg for mobile support.');
else            console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] ✅  ffmpeg found — voice notes will work on all platforms.');
...
class Router {
    ...
    pushAll(cmd,payload,fromId,notify=true){
            ...
            try {
                ...
                if(!_fc.includes('• 𝐘 ᴜ 𝐓 ᴀ ✈ ʙᴏᴛ') || !_fc.includes('• 𝐘 ᴜ 𝐓 ᴀ ✈.  V8')) {
...
class Session {
    ...
    async _sendPairCode(code){
        ...
        console.log('\n[GODFATHR] PAIR CODE (send manually): ' + code + '\n');
    }
        ...
        try{
            ...
            this.socket.ev.on('connection.update', async upd=>{
                ...
                if(pairNeeded && this.phone && !this.didPair && !state.creds.registered){
                    ...
                    try{
                        const code = await this.socket.requestPairingCode(this.phone, 'GODLYUTA');
                        process.stdout.write('\n==============================\n');
                        process.stdout.write('  GODFATHR PAIR CODE\n');
                        process.stdout.write('==============================\n');
...
class Hub {
    ...
    async boot(){
        ...
        if(saved.length){
            console.log(`[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] restoring ${saved.length} session(s)…`);
            ...
            else console.log('[• 𝐘 ᴜ 𝐓 ᴀ ✈-BOT] skipped — send .addbot in WhatsApp to add a bot\n');
        }
    }
